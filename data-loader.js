...
(function() {
    var templates = {
        ...
        'product-card': function(item) {
            ...
            var badges = (tags || pickBadge) ? '<div class="product-badges">' + tags + pickBadge + '</div>' : '';
            var topRow = '<div class="product-top-row"><div class="cookie-icon"><img src="' + iconSrc + '" alt="' + item.name + '" width="56" height="56" loading="lazy"></div>' + badges + '</div>';
            return '<div class="product-card' + (dailyPick ? ' daily-pick' : '') + '"><div class="product-card-inner"><div class="product-card-front">' + topRow + steamHTML + '<h3 class="product-name">' + item.name + '</h3><p class="product-desc">' + item.desc + '</p>' + (item.origin ? '<p class="product-origin">' + item.origin + '</p>' : '') + '<div class="product-footer"><span class="product-price">$' + parseFloat(item.price).toFixed(2) + '</span></div><div class="allergen-tags">' + allergens + '</div></div><div class="product-card-back"><h3 class="product-name">' + item.name + '</h3><div class="back-tasting"><span class="back-tasting-label">Tasting</span><span>' + (item.mood || '') + ' mood · ' + (item.intensity || '') + ' intensity · ' + (item.finish || '') + ' finish</span></div><ul class="ingredients-list">' + ingredients + '</ul>' + (item.origin ? '<p class="back-origin">' + item.origin + '</p>' : '') + '<div class="allergen-tags">' + allergens + '</div></div></div></div>';
        },
